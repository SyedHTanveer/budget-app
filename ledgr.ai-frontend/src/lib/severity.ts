...
// Lightweight direct usage classifier (ignores pace/reserves) for static ratios
export function classifyUsagePercent(usagePercent: number): SeverityLevel {
  if (!isFinite(usagePercent) || usagePercent < 0) return 'neutral'
  if (usagePercent < severityThresholds.usage.caution) return 'good'
  if (usagePercent <= severityThresholds.usage.danger) return 'caution'
  return 'danger'
}
